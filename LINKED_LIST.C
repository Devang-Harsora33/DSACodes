...
void create()
{
    ...
    for (i = 0; i < no_of_nodes; i++)
    {
        ...
        else
        {
            ...
            temp->next = newNode;
            // newNode->next = start; Circular LL
        }
    }
}
...
void del()
{
    ...
    if (temp->data == val)
    {
        ...
        /* if (temp->next == start)
        {
            start = NULL; // Set start to NULL when deleting the only node in the circular list
        }
        else
        {
            start = temp->next;
...
void display()
{
    ...
    struct Node *temp;
    temp = start;
    while (temp != NULL) // while (temp != start) Circular LL
    {
        if (temp->next != NULL) // if (temp->next != start) Circular LL
        {
            printf("%d -> ", temp->data);
        }
        else
